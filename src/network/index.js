...
export const sendAPIRequest = async ( endpoint, method, handleErrors, checkAuthorized = true, data ) => {
    ...
    let checkKeysForDates = (obj, reverse=false) => {
        if (obj instanceof Array) {
            obj.forEach(el => {
                if (el instanceof Object) checkKeysForDates(el, reverse);
            });
            return;
        }
        if (!(obj instanceof Object)) return;
        Object.keys(obj).forEach(key => {
            if (obj[key] instanceof Date && reverse) {
                obj[key] = new Date(obj[key].getTime()-timezoneOffset);
                return;
            } else if (obj[key] instanceof Object || obj[key] instanceof Array) {
                return checkKeysForDates(obj[key], reverse); 
            }
